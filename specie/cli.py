"""Specie command-line interface.

Every Cognis tool ships a CLI, structured output, and a library API. Run
`specie demo` for an end-to-end walkthrough on bundled sample data.
"""
...
def build_parser():
    p = argparse.ArgumentParser(
        prog="specie",
        description="Specie — Counter-Threat-Finance Attribution & Fusion Platform",
    )
    p.add_argument("--version", action="version", version=f"specie {__version__}")
