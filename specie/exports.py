...
Serialises findings and case files so Specie plugs into downstream
tooling:
...
def stix_bundle(findings, created: str = DEFAULT_TS) -> dict:
    ...
    rows = _as_dicts(findings)
    identity_id = _sid("identity", "Specie CTF Analytics")
    objects = [{
        "type": "identity", "spec_version": "2.1", "id": identity_id,
        "created": created, "modified": created,
        "name": "Specie CTF Analytics", "identity_class": "system",
    }]
    ...
    return {"type": "bundle", "id": _sid("bundle", "specie-findings:" + created),
            "objects": objects}
